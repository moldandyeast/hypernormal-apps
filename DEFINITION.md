...
Status: governing. Code implements this document; nothing in code may contradict it.
...
Anyone the law admits may hold a WebSocket to an app. On connect, the watcher receives the full state. After every state change, every watcher receives the full new state. After every amendment or rollback, every watcher receives the new charter. Watchers observe states in the order they occurred; a reconnecting watcher resumes from the current state. Watchers may relay small ephemeral signals to each other through the app (cursors, presence). A signal is bounded in size. These signals are never stored and never enter state.
