...
- Bridge: `options.tools` defaults to true where `document.modelContext` exists. Every public verb registers as a tool: name to name, description to description, `inputSchema` verbatim, `execute` delegates to the verb function. Every tool carries `untrustedContentHint: true`. One AbortController unregisters all tools on close. On a `charter` socket message, tools re-register, so amendments update live tool sets. Registration itself is gated same-origin by default: a verb's name and description are charter text, written verbatim into the document's own `modelContext`, so `connect()` only registers tools when the app is same-origin with the face document. `{tools: "cross-origin"}` opts back in; `{tools: false}` still disables registration entirely. Cross-origin Call and live-watch (charter fetch, socket, verbs) are unaffected.
- Named exports: `connect`, the pure `toolsFromCharter(charter)`, and the pure `mayRegisterTools(appUrl, docOrigin, opts)` for testing and reuse.
